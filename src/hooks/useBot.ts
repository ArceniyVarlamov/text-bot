    ...
    try {
      ...
      const response = await axios.post(
        `https://api.coze.com/open_api/v2/chat`, 
        {
          ...
          query: `Переписать текст, обеспечивая логический порядок и связь между предложениями, исключая специальные символы '[]{}()'. Убедитесь, что текст имеет плавные переходы и сохраняет смысл оригинала. Ваша задача - создать гармоничный и логичный текст${want}. Текст: \n${textData}`, 
          stream: false
        }, 
        {
          headers: {
